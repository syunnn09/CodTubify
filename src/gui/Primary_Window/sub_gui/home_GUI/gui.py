...
from tkinter import Canvas, Entry, Button, PhotoImage, Tk, END, Label
...
def on_download_completed(data: dict, canvas: Canvas):
    if data['status'] == 'finished':
        home_entrybox.delete(0, END)
        canvas.create_text(130, 285, text="download completed!", fill="#FFAAFF", tag="complete_text")

def download_thread(link, canvas: Canvas):
    a = threading.Thread(target=download_song(link, [lambda data: on_download_completed(data, canvas)]), daemon=True)
    a.start()


def Download_button_clicked(root: Tk, canvas: Canvas):
    canvas.delete('complete_text')
    ...
    if is_youtube_url(user_home_entry):
        try:
            ...
            root.after(100, download_thread(user_home_entry, canvas))
        ...
        root.after(100, download_thread(video_uri, canvas))
        #root.after(1000, lambda: toast.show_toast("Song Downloaded","Song Downloaded Successfully", duration=3,icon_path=relative_to_assets("codtubify.ico")))
...
def Home(parent):
    ...
    home_button_image_1 = PhotoImage(file=relative_to_assets("button_1.png"))
    Button(
        image=home_button_image_1,
        borderwidth=0,
        highlightthickness=0,
        command=lambda: threading.Thread(target=Download_button_clicked, args=(parent, canvas), daemon=True).start(),
        relief="flat",
        bg='#FFFFFF',
        activebackground='#FFFFFF'
    ).place(x=460.0, y=390.0, width=190.0, height=48.0)
