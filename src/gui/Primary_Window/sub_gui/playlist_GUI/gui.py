...
AUDIO_PATH = 'src/backend/temp_audio'

def playlist_play_button_clicked():
    currentsong = playlist.get(ACTIVE)
    print(f'playing: {currentsong}')
    try:
        mixer.music.load(os.path.join(AUDIO_PATH, currentsong))
        mixer.music.play()
    except Exception as e:
        print(f"error: {e}")


def song_seprator(obj: Listbox, lst: list[str]):
    for i in [l for l in lst if l.endswith('.mp3')]:
        obj.insert(END, i)

def song_seprator_thread(obj: Listbox, lst: list[str]):
...
def Playlist(parent: Tk):
    ...
    playlist_button_image_1 = PhotoImage(file=relative_to_assets("button_1.png"))
    Button(
    ...
    playlist_button_image_2 = PhotoImage(file=relative_to_assets("button_2.png"))
    Button(
    ...
    playlist = Listbox(
        ...
        selectmode=SINGLE,
        bg="SlateBlue4",
        selectbackground='SlateBlue4',
        selectforeground="SeaGreen",
        fg='white',
